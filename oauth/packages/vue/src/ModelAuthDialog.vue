...
const credentials = computed<OAuthCredential[]>(() => {
...
function updateCredential(credential: OAuthCredential, enabled: boolean, weight: number) {
  const provider = selectedProvider.value;
  ...
  localError.value = "";
  emit("update-credential", { providerId: provider.id, credentialId: credential.id, enabled, weight });
}
function removeCredential(credential: OAuthCredential) {
  const provider = selectedProvider.value;
  if (!provider || props.busy) return;
...
function credentialStatus(credential: OAuthCredential) {
