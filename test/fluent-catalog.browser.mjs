...
try {
  ...
  const checkedBoxVisual = checkedBox.locator(".fluent-checkbox__box");
  assert.equal(await checkedBoxVisual.locator(".fluent-checkbox__check").evaluate(node => node.tagName), "svg");
  assert.equal(await checkedBoxVisual.locator(".fluent-checkbox__check").getAttribute("data-icon"), "check");
  assert.equal(await background(checkedBoxVisual), accentHover, "checked checkbox hover lost its checked accent");
  assert.equal(await transform(checkedBoxVisual), "none", "checkbox hover changed its geometry");
  ...
  assert.equal(await background(partialBox.locator(".fluent-checkbox__box")), accentHover, "indeterminate checkbox hover lost its state accent");
