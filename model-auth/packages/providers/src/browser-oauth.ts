...
async function startCallbackListener(config: BrowserOAuthConfiguration, state: string, signal: AbortSignal, timeoutSignal: AbortSignal): Promise<CallbackListener> {
  ...
  const handler = (request: IncomingMessage, response: ServerResponse) => {
    ...
    const expectedHosts = new Set([redirect.host, `localhost:${redirect.port}`, `127.0.0.1:${redirect.port}`]);
    if (!expectedHosts.has(host) || request.method !== "GET") { response.writeHead(404).end(); return; }
    let url: URL;
...
function safeLoopbackRedirect(value: string, path: string): URL {
  ...
  if (redirect.protocol !== "http:" || !["localhost", "127.0.0.1"].includes(redirect.hostname) || !redirect.port || redirect.pathname !== path || redirect.username || redirect.password || redirect.search || redirect.hash) throw new BrowserOAuthError("response", "OAuth redirect URI is invalid.");
  return redirect;
}
