...
for (const target of targets) {
  ...
  const stage = resolve(output, ".stage", target.replaceAll("/", "-"));
  const manifest = JSON.parse(await readFile(resolve(source, "package.json"), "utf8"));
  const destination = resolve(output, packageDirectory(manifest.name));
...
const nativePath = resolve(output, "model-auth-native", nativeArchive);
await mkdir(resolve(output, "model-auth-native"), { recursive: true });
await cp(resolve(nativeSource, "target/package", nativeArchive), nativePath);
archives.push(nativePath);

const checksums = [];
for (const file of archives) checksums.push(createHash("sha256").update(await readFile(file)).digest("hex") + "  " + relative(output, file));
await writeFile(resolve(output, "SHA256SUMS"), checksums.join("\n") + "\n");
await rm(resolve(output, ".stage"), { recursive: true, force: true });
console.log("Prepared public archives: " + archives.map((file) => relative(output, file)).join(", "));
